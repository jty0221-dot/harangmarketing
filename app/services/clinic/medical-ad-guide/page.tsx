...
const BANNED_15: { no: string; what: string; where: string }[] = [
  { no: "제1호", what: "평가를 받지 않은 신의료기술 광고", where: "새 장비와 새 시술 소개 글에서 걸립니다" },
  {
    ...
    where:
      "후기와 체험단이 여기입니다. 환자분이 자발적으로 남기신 리뷰 자체가 아니라 그 리뷰를 광고에 옮겨 싣거나 대가를 드리고 받은 경우가 걸립니다",
  },
  ...
  { no: "제8호", what: "객관적인 사실을 과장", where: "통증 없는, 재발 없는 같은 단정" },
  {
    ...
    where:
      "전문과 선정의료기관이 여기서 걸립니다. 다만 전문병원 지정과 전문의 자격은 사실이라 그대로 쓰실 수 있습니다",
  },
  {
    no: "제10호",
  ...
  {
    no: "제12호",
    what: "외국인환자를 유치하기 위한 국내광고",
    where: "외국인환자 유치 자체가 아니라 그것을 국내에 광고하는 것이 금지입니다",
  },
  {
    no: "제13호",
    what: "소비자를 속이거나 잘못 알게 할 우려가 있는 방법으로 비급여 진료비용을 할인하거나 면제",
    where:
      "이벤트가와 패키지가 여기서 걸립니다. 진료비를 있는 그대로 안내하시는 것은 해당하지 않습니다",
  },
  {
    no: "제14호",
    what: "각종 상장과 감사장과 인증과 보증과 추천을 사용하거나 유사하게 표현",
    where: "인증 병원, 선정 같은 표기. 다만 조문에 예외 넷이 있어 아래에서 따로 봅니다",
  },
...
const COMMITTEES: { name: string; scope: string }[] = [
  ...
  {
    name: "치과의료광고심의위원회",
    scope: "치과의사, 치과의원, 치과병원, 종합병원의 치과와 그 개설자",
  },
  {
    name: "한방의료광고심의위원회",
...
const OPENING_CLINIC: { title: string; body: string }[] = [
  {
    ...
    body: "현수막과 벽보와 전단, 교통수단 광고, 전광판은 법률 본문에 이름이 그대로 있는 확실한 심의 대상입니다. 인쇄를 넘긴 뒤에 심의가 반려되면 인쇄물을 다시 만들어야 하고, 심의 없이 걸면 제56조 제2항 제11호가 됩니다.",
  },
  {
...
const GUIDE_FAQ: FaqItem[] = [
  ...
  },
  {
    ...
    a: "심의 유효기간은 심의를 받은 날부터 3년이고, 실무에서는 심의필증에 적힌 승인일자를 기준으로 셉니다. 계속 광고하시려면 만료 6개월 전에 재심의를 신청해야 합니다. 만료된 뒤에도 같은 광고가 걸려 있으면 그 순간부터 제56조 제2항 제11호의 심의를 받지 않은 광고가 됩니다. 위반은 발행일이 아니라 게시 중인 동안 계속 성립하기 때문에 지난 광고일수록 위험이 쌓입니다. 그래서 저희는 병원별로 심의기구와 심의번호와 승인일과 만료일을 대장에 적어 두고 만료 전에 먼저 알려드립니다.",
  },
  {
    q: "전후 사진을 올려도 되나요?",
      ...
      <main className="pt-[104px] md:pt-[108px] overflow-x-hidden">
            ...
            <div className="mt-7 rounded-2xl border border-white/10 bg-white/5 p-4 md:p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 space-y-2">
                  ...
                  <p className="text-xs md:text-sm text-gray-400 leading-relaxed">
                    이 자료는 저희가 원고를 넘기기 전에 보는 기준을 정리한 것이지 법률 자문이 아닙니다.
                    최종 판단은 자율심의기구와 관할 보건소에서 확인하시기 바랍니다.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8">
            ...
            <div className="mt-6 space-y-4 text-sm md:text-[15px] text-gray-700 leading-relaxed">
              ...
              <p>
                ...
                첫째, 법 제58조에 따른 의료기관 인증입니다. 둘째, 정부조직법상 중앙행정기관과
                특별지방행정기관과 부속기관, 지방자치법상 지방자치단체, 공공기관운영법상 공공기관에서 받은
                인증과 보증입니다. 셋째, 다른 법령에 따라 받은 인증과 보증입니다. 넷째, 세계보건기구와
                협력을 맺은 국제평가기구 인증 등 대통령령으로 정하는 광고입니다. 넷 중 어디에도 해당하지
                않으면 뺍니다.
              </p>
            </div>
          </div>
        ...
        <section id="expiry" className="py-12 md:py-16 bg-white scroll-mt-28">
          <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8">
            ...
            <div className="space-y-4 text-sm md:text-[15px] text-gray-700 leading-relaxed">
              <p>
                심의 유효기간은 심의를 받은 날부터 3년입니다. 실무에서는 심의필증에 적힌 승인일자를
                기준으로 셉니다. 계속 광고하시려면 유효기간 만료 6개월 전에 재심의를 신청해야 합니다.
                근거는 제57조 제8항과 제9항입니다.
              </p>
              <p>
        ...
        <section className="py-12 md:py-16 bg-gray-50">
          <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8">
            ...
            <div className="space-y-4 text-sm md:text-[15px] text-gray-700 leading-relaxed">
              ...
              <p>
                병원 광고에서 저희가 쓰지 않는 것이 하나 더 있습니다. 무료 검진과 비급여 진료비 할인과
                페이백처럼 진료비에 붙는 금전적 혜택 문구입니다. 저희 용역의 상담료가 0원인 것과는 다른
                자리입니다. 앞의 것은 원장님이 환자분께 드리는 것이고 뒤의 것은 저희가 원장님께 드리는
                것입니다.
              </p>
              <p>
                진료를 받아보세요, 상담을 받아보세요 같은 직접 권유 문구도 저희는 병원 원고에서 쓰지
                않습니다. 조문이 그 자체를 금지하는 것은 아니고, 대가나 할인과 붙는 순간 제27조 제3항으로
                넘어갈 여지가 생겨서 미리 빼는 것입니다.
              </p>
            </div>
          </div>
        </section>
