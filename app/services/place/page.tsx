...
import {
  byVolume, PLACE_RANK_AS_OF, PLACE_RANK_PAGE1_NOTE,
...
const WORK = [
  ...
  {
    icon: BarChart3,
    title: "오후 2~3시에 순위를 재서 조정합니다",
    body: "밀린 날이 있으면 무엇이 바뀌었는지 찾습니다. 저장해 둔 순위 스냅샷이 있어야 되돌릴 수 있습니다. 안 재면 밀린 것도 모릅니다.",
  },
];
...
const NEVER = [
  ...
  },
  {
    ...
    body: "순위는 저희가 정해진 시각에 재는 값이라 근거를 댈 수 있지만, 방문객과 매출은 저희 계측 대상이 아닙니다. 재지 않은 숫자는 말씀드리지 않습니다.",
  },
];
...
const FAQS: FaqItem[] = [
  ...
  {
    q: "얼마나 걸리나요?",
    a: `업종과 지역 경쟁 강도에 따라 다릅니다. 저희가 정해진 시각에 재서 남긴 기록으로 말씀드리면, 지금까지 1페이지에 올라온 기록 중 가장 빨랐던 건은 ${FASTEST.industry} 업종의 ${fmt(FASTEST)} 로 ${FASTEST.days}일이 걸렸습니다. 반대로 32일을 계측하고도 몇 계단만 움직인 기록도 있습니다. 상담 때 지금 몇 위인지부터 재 보고 대략의 기간을 말씀드립니다.`,
  },
  {
    q: "몇 위까지 올려 주시나요?",
    a: "몇 위까지 올려 드린다는 말을 화면에 쓰지 않습니다. 경쟁 매장이 무엇을 하는지, 네이버가 기준을 언제 바꾸는지는 저희가 정하지 못하기 때문입니다. 대신 정해진 시각에 순위를 재서 저장해 두고, 밀린 날에 무엇이 바뀌었는지 찾습니다.",
  },
  {
...
const LD = [
  webPageLd({
    ...
    description:
      "네이버 지도 등록은 무료이고 사장님이 직접 하실 수 있습니다. 저희는 그다음을 합니다. 키워드를 골라 넣고 오후 2~3시에 순위를 재서 밀린 키워드를 다시 손봅니다.",
  ...
  }),
  {
    ...
    description:
      "네이버 스마트플레이스 정보 최적화, 지역·업종 키워드 세팅, 경쟁 매장 분석, 사진 구성 가이드, 오후 2~3시 순위 계측과 조정. 순위는 저장해 둔 스냅샷 실측값으로만 보고하고 몇 위까지 올려 드린다는 말을 하지 않습니다.",
    ...
    <>
      ...
      <main className="pt-[104px] md:pt-[108px]">
        ...
        <section className="bg-gray-950 py-16 md:py-24">
          <div className="max-w-4xl mx-auto px-4 md:px-6">
            ...
            <p className="speakable mt-5 text-sm md:text-base leading-relaxed text-gray-300 max-w-2xl">
              네이버 지도 등록은 무료라 사장님이 직접 하실 수 있고, 절차는 아래에 적어 두었습니다. 저희는 그다음 키워드를 넣고 오후 2~3시에 순위를 재서 밀린 키워드를 다시 손봅니다. 몇 위까지 올려 드린다는 말 대신 실제로 잰 순위로만 말씀드립니다.
            </p>
        ...
        <AnswerBlock
          ...
          answer="지도에 올리는 등록은 무료라 직접 하시면 됩니다. 대행이 필요한 지점은 그다음입니다. 1페이지에는 광고 지면 3자리와 순위 1위부터 5위가 있는데, 광고는 예산을 멈추면 그날 내려오고 순위는 정보 완성도·키워드·사진·리뷰가 쌓여야 올라갑니다. 하랑마케팅은 오후 2~3시에 순위를 재서 밀린 자리를 되찾는 일을 합니다. 몇 위까지 올려 드린다는 말은 하지 않고 정해진 시각에 재서 저장한 실측값으로만 보고합니다. 상담 비용은 0원입니다."
          facts={[
            { label: "등록 비용", value: "0원 · 직접 가능" },
            { label: "순위 계측", value: "하루 한 번 · 잰 값만 보고" },
            { label: "상담", value: "0원" },
          ]}
        />
        ...
        <section className="py-14 md:py-20 bg-white">
          <div className="max-w-4xl mx-auto px-4 md:px-6">
            ...
            <p className="text-sm md:text-[15px] leading-relaxed text-gray-600 mb-8">
              다섯 가지입니다. 하나씩 채운 뒤 달라진 자리를 재서 기록하고,
              밀린 날은 원인부터 찾습니다.
            </p>
        ...
        <section className="py-14 md:py-20 bg-gray-50">
          <div className="max-w-5xl mx-auto px-4 md:px-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
              {[
                { icon: Store, label: "맡아온 매장", value: `${TRACK_TOTALS.stores}곳` },
                { icon: Layers, label: "업종", value: `${TRACK_TOTALS.trades}종` },
                { icon: ListOrdered, label: "맡아서 한 일", value: `${TRACK_TOTALS.workKinds}종` },
                { icon: LineChart, label: "순위 계측", value: "하루 한 번" },
              ].map((s) => (
        ...
        <PlaceRankCasesSection
          ...
          description={`오후 2~3시에 잰 순위를 그대로 옮겼습니다. 카드 하나가 키워드 하나여서, 한 매장이 키워드 셋을 올렸으면 기록도 셋으로 남습니다. ${PLACE_RANK_AS_OF} 계측분입니다.`}
