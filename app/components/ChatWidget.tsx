...
const KNOWLEDGE: BotEntry[] = [
  {
    ...
    answer:
      "홈페이지형 블로그 디자인은 STANDARD 20만원(PC), DELUXE 20만원(PC와 모바일), PREMIUM 30만원 세 가지로 진행되며, 대부분 DELUXE로 많이 하세요. 발주 후 초안까지 보통 3~4일 소요됩니다. 마음에 드는 레이아웃이나 참고 블로그가 있다면 카카오톡으로 링크를 남겨주세요. 포트폴리오와 발주 양식을 바로 보내드릴게요:)",
    actions: ["kakao"],
  },
  {
    keywords: ["포스팅", "블로그 글", "블로그글", "원고", "글 작성", "글작성", "대행", "기재", "발행"],
    answer:
      "블로그 포스팅은 1건 4만원이 기준이고, 보통 월 5~10건으로 진행하십니다. 다만 이 단가는 고정이 아니에요. 물량이 많거나 원고가 단순한 업종은 더 내려가고, 병의원처럼 의료광고 심의와 전문 용어 확인이 붙는 업종은 손이 훨씬 많이 들어 단가가 올라갑니다. 업종과 지역, 키워드를 보고 정확한 단가를 잡아드릴게요. 게시 후에는 링크를 모두 공유드립니다. 파워컨텐츠 등 광고 세팅은 별도이며 광고비는 네이버에 직접 결제됩니다.",
    actions: ["kakao", "freeCheck"],
  },
  {
    keywords: ["보장", "보장형", "확정", "무조건 올라", "안 되면 환불", "책임"],
    answer:
      "몇 위까지 올려 드린다는 말씀은 드리지 않아요. 순위를 정하는 건 네이버이고 경쟁 매장도 같이 움직이거든요. 대신 순위를 꾸준히 기록해서 밀린 날에 무엇이 바뀌었는지 찾고, 잰 숫자를 그대로 보여드려요. 원하시는 키워드를 카카오톡으로 남겨주시면 지금 몇 위인지부터 확인해서 알려드릴게요.",
    actions: ["kakao"],
  },
  {
  ...
  },
  {
    ...
    answer:
      "네, 제작만이 아니라 노출 관리(관리대행)까지 저희가 직접 합니다. 본업이 마케팅 대행이고 블로그 디자인 제작은 그중 한 부분이에요. 관리는 블로그 포스팅 1건 4만원이 기준이고 보통 월 5~10건으로 진행하는데, 업종과 원고 난이도에 따라 조정됩니다(병의원처럼 심의가 걸리는 업종은 더 붙습니다). 발행 후 게시 링크를 전부 보내드려 어떤 글이 어떻게 올라갔는지 직접 확인하실 수 있습니다:)",
    actions: ["kakao", "freeCheck"],
  },
  {
  ...
  },
  {
    ...
    answer:
      "트래픽이라는 말이 두 가지를 가리켜서 나눠서 말씀드릴게요. 하나는 프로그램이나 매크로로 기계가 자동 클릭하는 거예요. 이건 저희가 쓰지 않습니다. 네이버가 금지하고 있고, 위험도 여기서 나오거든요. 사람이 아니라서 들어와서 아무것도 안 하고 나가는데 네이버가 그걸 봅니다. 한 번 걸리면 복구가 안 되고 그동안 쌓으신 리뷰까지 같이 묻혀요. 다른 하나는 리워드예요. 실제 사람이 직접 검색해서 들어와 보고 나가는 겁니다. 이건 씁니다. 솔직하게 말씀드리면 지금 플레이스 상위노출을 하려면 이 작업이 일부는 필요해요. 글만 쌓아서는 속도가 안 납니다. 지금 붙여둔 매장들 반응도 좋고 점수도 계속 올라가고 있어요. 물론 이것만으로 되는 건 아니라서, 글이랑 리뷰가 같이 쌓여야 올라간 자리가 유지됩니다.",
    actions: ["kakao"],
  },
  {
  ...
  },
  {
    ...
    answer:
      "전국 어디든 가능해요. 지금까지 맡은 매장이 부산·대구·제주·세종을 포함해 14개 시·도에 있어요. 사무실은 경기 북서부라 이 근처는 방문 상담도 되고, 다른 지역은 카카오톡·전화·화상으로 진행해요. 방문 상담이 필요하시면 위치 안내드릴게요:)",
    actions: ["kakao"],
  },
];
