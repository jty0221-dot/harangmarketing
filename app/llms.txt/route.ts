...
/*
 * 이 파일에 손으로 적은 문구를 마지막으로 고친 날 (2026-09-24 · 요청 71). 문구를 고치면 같이 바꾼다.
 ...
 */
const LLMS_TEXT_UPDATED = "2026-09-24";
...
function companyYearText(): string {
  ...
  const founded = Number(SITE.foundingDate.slice(0, 4));
  return `${year}년 기준 ${year - founded + 1}년차`;
}
...
export async function GET() {
  ...
  const services = [
    ...
    `**카카오맵 마케팅**: 카카오맵 플레이스 등록·최적화. 카카오맵으로 가게를 찾는 손님까지 확보.`,
    `**맘카페 바이럴**: 지역 맘카페 실사용자 후기형 노출. 학원·카페·음식점·네일 업종에 특화.`,
...
- 대신 순위를 매일 저장해 밀린 날에 무엇이 바뀌었는지 그날 안에 찾고, 잰 숫자를 매월 리포트에 그대로 적는다.
...
- [네이버 플레이스 상위노출 · 지도 등록 대행](${B}/services/place): 정보 완성도 · 키워드 · 경쟁 매장 분석 · 사진 구성 · 매일 순위 계측 다섯 단계 · 몇 위까지 올려 드린다는 말을 하지 않음
...
- [마케팅 성공 사례](${B}/cases): 매일 잰 플레이스 순위 기록 발췌 · 맡아 온 매장 ${TRACK_TOTALS.stores}곳을 업종과 맡은 일로 나눠 적음
