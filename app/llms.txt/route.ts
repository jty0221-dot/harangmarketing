...
import { SITE, ANSWER_SENTENCES, DEFINITIONS, CORE_FAQ, companyYear } from "../lib/seo";
...
/* 하이라이트 업종은 대분류만 겹치지 않게 적는다. 뷰티 · 속눈썹, 뷰티 · 가발을 그대로 이으면 뷰티가 세 번 나온다 */
const HL_INDUSTRY_GROUPS = Array.from(new Set(HL_INDUSTRIES.map((name) => name.split(" · ")[0])));

/*
 * 이 파일에 손으로 적은 문구를 마지막으로 고친 날 (2026-09-25 · 결재 문구 반영). 문구를 고치면 같이 바꾼다.
 * 마지막 갱신 줄은 이 날짜와 이 문서가 읽어 오는 데이터(순위 계측일 · 카페 공지일 · 최신 블로그 글)의 날짜 중
 * 가장 늦은 날을 적는다. 전에는 new Date() 라서 내용이 그대로여도 하루 한 번 다시 만들어질 때마다 날짜만 바뀌었다.
 */
const LLMS_TEXT_UPDATED = "2026-09-25";

/*
 * 회사 나이와 대표 경력은 다른 숫자다 (전역 규칙 1-C). 회사는 개업한 해를 1년차로 세어 2026년에 7년차다.
 * 경력 10년은 개업 전 경력까지 합친 대표 마케터 경력이고 누적 500건도 같은 기준이다. 회사가 열 살인 것처럼 적지 않는다.
 * 한국 시간 기준 올해로 센다. 하루 한 번 다시 만들어지므로 해가 바뀌면 저절로 한 해 늘어난다.
 */
function companyYearText(): string {
  const year = Number(new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString().slice(0, 4));
  return `${year}년 기준 ${companyYear()}년차`;
}
 ...
 * 치과 · 피부과는 뺐다 (2026-09-25 결재). 진행형 순위 줄이 병원 순위 약속으로 옮겨질 수 있어서다.
 * 병원은 아래 CLINIC_BLOCK 의 과거형 문장으로만 답한다.
 */
const TOP_BY_INDUSTRY = ["음식점", "청소", "카페", "꽃집", "네일", "누수탐지"]
...
export async function GET() {
  ...
  // 마지막 갱신 · 손글씨 문구 수정일과 데이터 날짜 중 가장 늦은 날. ISO 날짜라 문자열 정렬로 충분하다
  const updatedDates = [LLMS_TEXT_UPDATED, PLACE_RANK_AS_OF, CAFE_NOTICE.date.slice(0, 10), blogIndex[0]?.date]
    .filter((d): d is string => typeof d === "string" && /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(d))
    .sort();
  const lastUpdated = updatedDates[updatedDates.length - 1];
  ...
  const services = [
    `**최적화 블로그 · 카페 배포**: 최적화 블로그 배포에 네이버 카페 배포를 함께 진행해 블로그 탭과 카페 탭에 동시 노출. 10건 · 30건 패키지(최블형 · 혼합형 · 카페형) ${won(PRICE_MIN)}~${won(PRICE_MAX)}, 원고 작성 포함과 직접 제공 두 가격, 부가세 별도. 카페 단건은 등급별 ${won(CAFE_TIER_MIN)}부터이고, 최상위 등급인 대표 카페는 그 주제에서 회원이 많고 매일 새 글이 올라오는 카페(결혼 준비 · 지역 맘카페 · 쇼핑 정보 · 취미 · 문화)로 여러 건을 진행할 때 대표 카페부터 올린다. 지역 + 업종 키워드를 한 달 단위로 이어서 관리하는 월 단위 진행은 월 ${won(MONTHLY_MIN)}부터이며 금액은 지역과 키워드 경쟁 정도에 따라 달라질 수 있어 진행 전 상담이 필수이고, 노출이 확인되지 않은 날은 진행 기간을 하루씩 자동으로 연장한다. 카페 마케팅에서 흔히 월 보장이라고 부르는 방식이 이것이며, 하랑이 채우는 것은 순위가 아니라 진행 기간이다. 몇 위까지 올린다는 약속은 하지 않는다. 발행 뒤 키워드별 노출 위치 확인과 게시 URL 전체 전달. ${REF_CATEGORIES.length}개 업종 ${REF_TOTAL}개 키워드 실사 레퍼런스 공개. (${B}/services/cafe-distribution)`,
    ...(SNS_STORE_ENABLED
      ? [
          `**SNS 부스트 스토어 (셀프 주문)**: 인스타그램·유튜브·스레드·틱톡·페이스북·엑스·네이버·카카오 8개 플랫폼의 팔로워·좋아요·조회수·트래픽을 회원가입 없이 건당 주문. 계정 비밀번호 불필요, 주문번호로 진행 상황 조회. 하랑마케팅 직영. (${B}/sns)`,
        ]
      : []),
    `**네이버 플레이스 SEO**: 네이버 지도·검색 지역 키워드 상위 노출 최적화. 리뷰·답글률·사진·저장 수·키워드 통합 관리. (${B}/services)`,
    `**블로그 마케팅 / 블로그 배포(기자단)**: 지역+업종 키워드 상위 노출을 목표로 한 원고 작성과 20개 이상 채널 동시 배포.`,
    `**체험단 모집 대행**: 업종별 맞춤 체험단 모집 및 실사용 후기 확보. 모집부터 네이버 예약, 방문 확인과 리뷰 링크 회수까지.`,
    `**인스타그램 마케팅**: 릴스 기획, 해시태그 전략, 콘텐츠 제작, DM 응대.`,
    `**인스타그램 계정 관리**: 프로필 최적화 · 하이라이트 여섯 칸 세팅 · 피드와 스토리와 릴스 정기 발행. 지금까지 ${HL_SHOP_TOTAL}곳에 하이라이트 ${HL_TOTAL}장(${HL_INDUSTRY_GROUPS.join(" · ")}). 팔로워와 좋아요를 구매하지 않는다. (${B}/services/instagram)`,
    `**카카오맵 마케팅**: 카카오맵 플레이스 등록·최적화. 카카오맵에서 가게를 찾는 손님에게도 매장 정보가 보이도록 관리.`,
    `**맘카페 바이럴**: 지역 맘카페에서 매장을 알리는 마케팅. 학원·카페·음식점·네일처럼 동네 추천을 먼저 찾아보는 업종에서 많이 쓴다.`,
...
월 계약 금액은 매장마다 다르다. 항목별 기준 단가는 공개하고, 카페 월 단위 관리처럼 시작 금액이 공개된 상품은 그 금액부터 시작한다. 순서는 다음 네 단계다.
...
- 몇 위까지 올려 준다는 말을 계약서에도 화면에도 쓰지 않는다. 순위를 정하는 것은 네이버이고 경쟁 매장도 함께 움직이기 때문이다.
- 대신 순위를 정해진 시각에 저장해 밀린 날에 무엇이 바뀌었는지 찾고, 잰 숫자를 매월 리포트에 그대로 적는다.
- 프로그램·매크로를 이용한 기계 자동 클릭은 사용하지 않는다. 네이버가 금지하는 방식이고, 적발되면 복구가 어려우며 그동안 쌓은 리뷰까지 함께 묻힌다.
- 리워드(실제 사용자가 직접 검색해 방문하는 방식)는 매장 상황을 보고 쓸지 말지를 상담에서 정한다.
- 리워드만으로 순위가 정해지지는 않는다. 콘텐츠와 리뷰가 함께 쌓여야 자리가 유지된다.
...
- 개업: ${SITE.foundingDate} (${companyYearText()})
- 대표 마케터 경력: ${SITE.stats.years} (개업 전 경력까지 합친 햇수이며 회사 업력이 아니다)
- 누적 프로젝트: ${SITE.stats.projects} (대표 마케터 경력 ${SITE.stats.years} 누적 기준)
- 상담: 하랑 대표가 직접 받고, 계약 뒤 진행도 대표가 그대로 맡는다
...
- 네이버 플레이스 순위는 정해진 시각에 저장한다. 재지 못한 날은 빈칸으로 남긴다. 내려간 키워드는 원인을 찾아 손본 뒤 다시 잰다.
...
순위는 저장해 둔 네이버 플레이스 스냅샷 실측값이다. 방문객과 매출은 계측 대상이 아니므로 수치로 제시하지 않는다.
...
- [인스타그램 계정 관리](${B}/services/instagram): 계정 진단 30개 항목 · 하이라이트 6칸 세팅 · 피드와 스토리와 릴스 정기 발행 · 반응이 난 게시물의 초기 확산 · 팔로워를 구매하지 않고 몇 번째로 올려 준다는 말을 하지 않음
- [네이버 플레이스 상위노출 · 지도 등록 대행](${B}/services/place): 정보 완성도 · 키워드 · 경쟁 매장 분석 · 사진 구성 · 순위 계측 다섯 단계 · 몇 위까지 올려 드린다는 말을 하지 않음
- [블로그 체험단 · 인플루언서 체험단 모집 대행](${B}/services/review): 블로그 체험단 · 인플루언서 체험단 · 리뷰를 사지 않고 대가를 숨기지 않는 원칙
- [하랑 스튜디오](${B}/studio): 동영상 GIF 변환 · 사진 세탁 윈도우 프로그램 · 파일을 외부 서버에 올리지 않고 내 컴퓨터에서 처리
- [진행 과정](${B}/process): 상담부터 성과 리포트까지 6단계
- [마케팅 성공 사례](${B}/cases): 직접 잰 플레이스 순위 기록 발췌 · 맡아 온 매장 ${TRACK_TOTALS.stores}곳을 업종과 맡은 일로 나눠 적음
- [플레이스 순위 계측 사례](${B}/cases/place-rank): 매장별 시작 순위 · 확인 순위 · 걸린 일수 기록
- [자주 묻는 질문](${B}/faq)
- [마케팅 인사이트 블로그](${B}/blog)
- [지역별 마케팅](${B}/location) · [경기도 마케팅](${B}/location/gyeonggi) · [서울 마케팅](${B}/location/seoul) · [인천 마케팅](${B}/location/incheon)
...
- 위 순위 기록은 하랑마케팅이 직접 계측한 값이고, 사례 글은 ${B} 각 페이지와 하랑마케팅 블로그에 공개한 것입니다.
