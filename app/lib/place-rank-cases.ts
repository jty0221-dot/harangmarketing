...
/**
 * 언제 잰 값인지 — 화면에 적는 시각을 한 곳에서만 만든다.
 *
 * 순위 계측 예약작업(place-rank-watch)은 매일 10:00 과 14:00 두 번 돈다.
 * 화면에 싣는 것은 오후 회차다 — 오전 값은 그날 순위가 아직 덜 움직인 값이다.
 * 2026-09-17 (목) 대표 지시 : 업체는 매일 현시각 오후2~3시를 기준으로 만들고.
 * 하루 한 번 이라고만 적던 자리를 전부 이 문장으로 바꾼다. 재는 시각을 적어야 잰 값으로 읽힌다.
 * 2026-09-25 (금) 결재 : 09/21 · 09/22 스냅샷이 비어 있어 「매일」은 뺐다. 시각만 적는다.
 */
export const PLACE_RANK_MEASURE_TIME = `오후 2~3시`;

/** 화면 · JSON-LD · llms.txt 가 같은 문장을 쓰도록 한 곳에서 만든다 */
export const PLACE_RANK_NOTE =
  `애드랭크 순위 계측 데이터를 기준으로 한 네이버 플레이스 실측값입니다. ` +
  `${PLACE_RANK_MEASURE_TIME} 사이에 재서 그날 값으로 남기고, 재지 못한 날은 빈칸으로 둡니다. ` +
...
/**
 * 화면에 올리지 않는 것 — 카드 목록의 제외 기준.
 *
 * /cases 가 「계측 방법과 제외 기준 보기」 라는 문구로 이 페이지를 걸어 두고 있는데
 * 계측 방법(PlaceRankMonitoring)만 있고 제외 기준은 화면에 없었다. 링크가 약속한 쪽을 채운다.
 * (2026-09-17 (목) · 매스티지 뉴스레터 기준 홈페이지 점검에서 나온 공백)
 *
 * 다섯 줄 전부 이 파일과 생성기가 이미 그렇게 돌고 있는 것을 옮긴 것이다. 새로 한 약속이 아니다.
 *   1) 5위 밖 — SHOW_WITHIN 필터 (대장에서 지우지 않고 화면에서만 거른다)
 *   2) 상호 · 지역명 — 규칙 1 · 2 · PLACE_RANK_LABEL_NOTE
 *   3) 최신 스냅샷 — 규칙 5 · PLACE_RANK_NOTE
 *   4) 방문객 · 매출 — PLACE_RANK_LABEL_NOTE 후단
 *   5) 몇 위까지 — 헌장 C-36
 *
 * 2026-09-17 (목) 대표 지시로 두 줄을 화면에서 걷었다 — 떨어진 건과 병 · 의원이다.
 *   문장만 걷었고 동작은 그대로다. 규칙 6 의 trend 필터도 규칙 4 의 pendingReview 도 생성기에서 계속 돈다.
 *   고객이 굳이 알 필요 없는 우리 쪽 사정이라 안 적는 것이지, 안 거르는 것이 아니다.
 * 이 문장을 고칠 때는 위 동작도 같이 고친다. 한쪽만 고치면 적힌 것과 도는 것이 갈린다.
 */
export const PLACE_RANK_EXCLUSIONS_NOTE =
  `이 페이지의 카드는 계측 기록 전부가 아닙니다. 아래 기준에 걸리는 건은 화면에 올리지 않습니다.`;

export const PLACE_RANK_EXCLUSIONS: { title: string; body: string }[] = [
  {
    title: `${SHOW_WITHIN}위 밖은 화면에서 거릅니다`,
    body:
      `네이버 플레이스 1페이지는 순위 1~${SHOW_WITHIN}위입니다. ${SHOW_WITHIN}위 밖으로 내려간 키워드는 카드에서 빠집니다. ` +
      `기록 자체를 지우는 것이 아니라 화면에서만 거르기 때문에, 다음 계측에서 순위가 돌아오면 그 카드가 다시 올라옵니다.`,
  },
  {
    title: `상호와 지역명은 적지 않습니다`,
    body:
      `업종과 행정단위까지만 적습니다. 상호는 서면으로 공개에 동의하신 곳만 따로 적습니다.`,
  },
  {
    title: `최고 기록이 아니라 지금 순위입니다`,
    body:
      `한 번 찍힌 최고 순위를 적지 않습니다. ${PLACE_RANK_AS_OF} 계측분 스냅샷에 있는 현재 순위를 그대로 옮깁니다.`,
  },
  {
    title: `방문객과 매출은 적지 않습니다`,
    body:
      `저희가 재는 것은 순위입니다. 방문객 수 · 예약 건수 · 매출은 계측 대상이 아니어서 수치로 적지 않습니다. ` +
      `재지 않은 숫자를 적으면 그 숫자가 약속이 됩니다.`,
  },
  {
    title: `몇 위까지 올려 드린다는 말을 하지 않습니다`,
    body:
      `이 페이지에 있는 것은 전부 이미 잰 값입니다. 앞으로 몇 위가 된다는 말은 적지 않습니다. ` +
      `업종과 지역 경쟁 강도에 따라 결과는 달라집니다.`,
  },
];
