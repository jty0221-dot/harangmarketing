 ...
 *
 * 일곱 줄 전부 이 파일과 생성기가 이미 그렇게 돌고 있는 것을 옮긴 것이다. 새로 한 약속이 아니다.
 *   1) 떨어진 건 — 규칙 6 · trend 는 상승과 유지 두 값뿐이다
 *   2) 5위 밖 — SHOW_WITHIN 필터 (대장에서 지우지 않고 화면에서만 거른다)
 *   3) 병 · 의원 — 규칙 4 · 생성기가 pendingReview 로 빼낸다
 *   4) 상호 · 지역명 — 규칙 1 · 2 · PLACE_RANK_LABEL_NOTE
 *   5) 최신 스냅샷 — 규칙 5 · PLACE_RANK_NOTE
 *   6) 방문객 · 매출 — PLACE_RANK_LABEL_NOTE 후단
 *   7) 몇 위까지 — 헌장 C-36
...
export const PLACE_RANK_EXCLUSIONS: { title: string; body: string }[] = [
  {
    title: `떨어진 건은 싣지 않습니다`,
    body:
      `올라간 건과 지키고 있는 건만 싣습니다. 내려간 키워드는 카드로 만들지 않습니다. ` +
      `3위에서 3위로 그대로인 건은 올랐다고 적지 않고 지키고 있다고 적습니다.`,
  },
  {
  ...
  {
    title: `병 · 의원 사례는 검수 전까지 올리지 않습니다`,
    body:
      `의료광고는 병원 명의로만 나갑니다. 병 · 의원 키워드는 의료법 검수를 통과하기 전까지 ` +
      `목록을 만드는 단계에서 빠지므로 이 페이지에 올라오지 않습니다.`,
  },
  {
