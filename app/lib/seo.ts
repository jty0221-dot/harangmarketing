...
export const ANSWER_SENTENCES = {
  ...
  clinic:
    `하랑마케팅은 병·의원 전담 담당을 따로 두고 의료법 검수를 거친 뒤에 원고를 내보내는 마케팅 대행사입니다. 의료광고 사전심의 대상 여부를 원고를 쓰기 전에 판정하고, 심의를 받은 광고는 승인일부터 3년인 유효기간을 대장으로 관리합니다. ${CLINIC_NOTE} 앞으로 몇 위가 될지는 말씀드리지 않습니다. 방문 환자와 예약 건수 · 매출은 저희가 계측할 수 있는 값이 아니라서 수치로 제시하지 않습니다.`,
  blogPriceByIndustry:
