...
export const TRACK_TOTALS = {
  ...
  /** 맡아서 한 일의 종류 수 */
  workKinds: new Set(ALL_ITEMS.flatMap((i) => i.work)).size,
};

/*
 * region 과 status 는 데이터에 그대로 두고 화면에서만 걷었다 (2026-09-07 (월) 대표 지시)
 *   「지금도 관리중은 굳이 보여서 계약이 유지 되지 않는것을 보여주는 것 같아 안보이게 제외해
 *    그리고 특정 지역 상호는 안보이게 하고」
 *
 * 진행 중인 곳만 표시하면 나머지가 끝난 곳으로 읽힌다. 맡아온 일은 끝나도 맡았던 사실이 남는데
 * 화면이 그것을 계약이 끊긴 자리처럼 보여주고 있었다. 그래서 진행 상태를 아예 세지 않는다.
 *
 * 지역은 업종과 겹치면 업체가 드러난다. 광역 단위까지만 적어도 업종이 드문 곳은 한 곳으로 좁혀진다.
 * 항목 옆 지역 배지를 걷으면서 그것을 세던 regions 도 같이 걷었다.
 * 근거가 화면에 없는 숫자만 남겨 두면 어디서 나온 값인지 아무도 대지 못한다.
 *
 * 두 필드를 타입에서 지우지는 않는다. 계약 대장에서 옮겨 적은 사실이고,
 * 다시 보여줄 날이 오면 데이터를 새로 모으지 않고 화면만 되돌리면 된다.
 */
